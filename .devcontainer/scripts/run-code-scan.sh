...
then
    ...
    /d:sonar.sarif.path=$WORKING_DIR/horusec-results.sarif,$WORKING_DIR/results_sarif.sarif #,$WORKING_DIR/bpa-results.sarif
    ...
    checkov -d $WORKING_DIR -o sarif --output-file-path=$WORKING_DIR #--skip-framework bicep arm
    # Template analyzer currently has a bug (https://github.com/Azure/template-analyzer/issues/307) that provides wrong line numbers
    # It won't be used untill this bug is fixed, because SonarQube fails to import issues on non-existent lines
